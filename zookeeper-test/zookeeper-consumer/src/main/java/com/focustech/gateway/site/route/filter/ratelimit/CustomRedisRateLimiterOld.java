...
public class CustomRedisRateLimiterOld extends AbstractRateLimiter<CustomRedisRateLimiterOld.Config> implements ApplicationContextAware {
    @Deprecated
    ...
    public CustomRedisRateLimiterOld(ReactiveRedisTemplate<String, String> redisTemplate, StringRedisTemplate redisTemplate2,
    ...
    public CustomRedisRateLimiterOld(int defaultReplenishRate, int defaultBurstCapacity) {
    ...
    public void setApplicationContext(ApplicationContext context) throws BeansException {
        if (initialized.compareAndSet(false, true)) {
            ...
            this.redisTemplate2 = context.getBean("stringRedisTemplate", StringRedisTemplate.class);
...
//                });
        for (ApiRateLimit limit : rateLimits) {
            Mono<Response> response = isAllowed(getKeyId(routeId, limit, exchange), limit.getReplenishRate(), limit.getReplenishRate(), getEpochTime(limit.getTimeUnit()));
...
//            List<Long> result = this.redisTemplate2.execute(this.script, new Jackson2JsonRedisSerializer(mapper.getTypeFactory().constructCollectionType(List.class, String.class)),
//                    new Jackson2JsonRedisSerializer<>(mapper.getTypeFactory().constructCollectionType(List.class, Long.class)), keys, scriptArgs);
           /* List<Long> result = this.redisTemplate2.execute(this.script, new Jackson2JsonRedisSerializer(mapper.getTypeFactory().constructCollectionType(List.class, String.class)),
                    new Jackson2JsonRedisSerializer<>(mapper.getTypeFactory().constructCollectionType(List.class, Long.class)), keys, replenishRate + "", burstCapacity + "",
                    epochTime + "", "1");*/
            RedisSerializer<String> stringRedisSerializer = redisTemplate2.getStringSerializer();
//            redisTemplate2.getConnectionFactory().getClusterConnection().scriptLoad(this.script.getScriptAsString().getBytes());
            List<Long> result = this.redisTemplate2.execute(this.script,stringRedisSerializer, new Jackson2JsonRedisSerializer(mapper.getTypeFactory().constructCollectionType(List.class, String.class)),
                    keys, "1", "2",
                    epochTime + "", "1");
            log.debug("script result:" + JSON.toJSONString(result));
